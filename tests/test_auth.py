def test_login_success(client, test_user, app_context):
...
def test_logout(auth_client, app_context):
    response = auth_client.get('/logout', follow_redirects=True)
    assert response.status_code == 200
    assert b'Login' in response.data

def test_protected_route_redirect(client, app_context):
    response = client.get('/', follow_redirects=True)
    assert response.status_code == 200
    assert b'Login' in response.data

def test_settings_update_username(auth_client, app_context):
...
def test_settings_update_password(auth_client, app_context):
